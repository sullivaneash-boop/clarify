...
Do not add `GEMINI_API_KEY`, the Supabase `service_role` key, an `sb_secret_` key, or any Supabase secret key to Vercel. Gemini belongs in Supabase secrets only.
